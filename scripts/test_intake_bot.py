...
class DeskPicksViewTests(unittest.TestCase):
    ...
    def test_same_card_update_edits_only_the_clicked_message(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                state["cards"]["picks:401"]["message_id"],
                30,
            )

    def test_stale_callback_cannot_replace_the_game_card(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            config = SimpleNamespace(
                state_path=Path(tmp) / "moe_desk_state.json",
                chat_id="-1001",
            ...
            self.assertEqual(
                saved["cards"]["picks:401"]["message_id"],
                40,
            )
...
class DeskModelHelpersTest(unittest.TestCase):
    """The desk's ✅/❌ review flow was removed 2026-09-09/10 (review is
    automatic at generation); what remains here is the DM stub helper."""

    def test_game_stub_carries_the_event_id(self) -> None:
