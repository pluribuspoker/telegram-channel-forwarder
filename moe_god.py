...
def reason_reference_text(request: dict[str, Any]) -> str:
    ...
    way a reason would write them and appended to the request text — and so
    are the records the request states in prose ("won 132, lost 84, and
    tied 1" -> 132-84-1), cohort shares ("won 26 of 49 games" -> 26-23),
    the celebrity ballot ("votes {'X': 3, 'Y': 2}" -> 3-2), the forecaster
    consensus ("4 of 5 forecasters ... 0 ranked ... and 1 tied" -> 4-0-1),
    and the prior-win comparison ("7 prior-season wins to Y's 3" -> 7-3).
    """
    ...
    for voice in request.get("voices", []):
        ...
        for kind in ("legs", "ats", "ou"):
            ...
            if all(key in tally for key in ("w", "l", "p")):
                ...
                # The cohort a structured tally implies: "its O/U record
                # is 5-10 over 15 games" cites w+l+p, which "resolved"
                # does not carry when pushes differ (live rejection,
                # 2026-09-20).
                derived.append(
                    f"{tally['w'] + tally['l'] + tally['p']} games"
                )
    ...
    counts(request)
    lowered = _reference_form(text)
    for match in _RECORD_PATTERN.finditer(lowered):
        derived.append(
            f"{sum(int(group) for group in match.groups() if group)} games"
        )

    # Records the request states in prose rather than W-L notation. A
    # judge re-notating one is citing the request, and 30 live responses
    # were rejected for exactly that on the first full Sunday slate
    # (2026-09-13). LLM voices SAMPLE their factor prose, so one fact
    # arrives in several spellings across generations — each rule below
    # covers a phrasing family observed in live payloads, not one string.
    def prose_record(wins: int, losses: int, ties: int | None = None) -> None:
        derived.append(f"{wins}-{losses}")
        if ties is not None:
            derived.append(f"{wins}-{losses}-{ties}")
        derived.append(f"{wins + losses + (ties or 0)} games")

    # Win-total gap buckets: "won 132, lost 84, and tied 1" -> 132-84-1.
    for match in re.finditer(
        r"won (\d+), lost (\d+)(?:,? and tied (\d+))?", lowered
    ):
        wins, losses, ties = match.groups()
        prose_record(int(wins), int(losses), int(ties) if ties else None)
    # The same bucket record in noun order: "went 106 wins, 83 losses,
    # and 0 ties over 189 games" -> 106-83 and 106-83-0. Three live
    # rejections of the same re-notated record stalled two WAS@DAL
    # committees on 2026-09-20.
    for match in re.finditer(
        r"(\d+) wins?, (\d+) loss(?:es)?(?:,? and (\d+) ties?)?", lowered
    ):
        wins, losses, ties = match.groups()
        prose_record(int(wins), int(losses), int(ties) if ties else None)
    # Exact prior-win pairs: "split 2 wins and 2 losses, 0 ties" -> 2-2.
    for match in re.finditer(
        r"split (\d+) wins? and (\d+) loss(?:es)?(?:, (\d+) ties?)?", lowered
    ):
        wins, losses, ties = match.groups()
        prose_record(int(wins), int(losses), int(ties) if ties else None)
    # Cohort shares: "won 26 of 49 games" -> 26-23 (and the 23-26 order a
    # reason may prefer); also vote shares like "covering 5 of 6".
    for match in re.finditer(r"(\d+) of (\d+)", lowered):
        part, whole = int(match.group(1)), int(match.group(2))
        if part <= whole:
            derived.append(f"{part}-{whole - part}")
            derived.append(f"{whole - part}-{part}")
            derived.append(f"{whole} games")
    # The celebrity ballot: "votes {'green bay packers': 3, 'minnesota
    # vikings': 2}" -> 3-2 either way around.
    for match in re.finditer(r"votes \{[^}]*\}", lowered):
        ballots = re.findall(r": (\d+)", match.group(0))
        for index, first in enumerate(ballots):
            for other, second in enumerate(ballots):
                if index != other:
                    derived.append(f"{first}-{second}")
    # The forecaster consensus sentence, both phrasings: "4 of 5
    # forecasters with both teams ranked X higher, 0 ranked Y higher, and
    # 1 tied" and "4 forecasters projecting more away wins, 0 more home
    # wins, and 1 tie" -> 4-0-1.
    for match in re.finditer(
        r"(\d+) of \d+ forecasters[^\"]{0,160}?"
        r"(\d+) ranked[^\"]{0,80}?,? and (\d+) tied",
        lowered,
    ):
        higher, lower, tied = (int(group) for group in match.groups())
        prose_record(higher, lower, tied)
    for match in re.finditer(
        r"(\d+) forecasters projecting more \w+ wins, "
        r"(\d+) more \w+ wins, and (\d+) ties?",
        lowered,
    ):
        higher, lower, tied = (int(group) for group in match.groups())
        prose_record(higher, lower, tied)
    # "4 of 5 forecasters gave ... versus 1 for the away team, with 0
    # tied" -> the complement above grounds 4-1; this grounds 4-1-0.
    for match in re.finditer(
        r"(\d+) of (\d+) forecasters[^\"]{0,160}?with (\d+) tied", lowered
    ):
        higher, total, tied = (int(group) for group in match.groups())
        if higher + tied <= total:
            prose_record(higher, total - higher - tied, tied)
    # The ballot in labeled-count form, both spellings sampled by the
    # win-total voice: "(home_team_higher_count 5, away_team_higher_count
    # 0, tied_count 0)" and "(home-higher count 4, away-higher 0, tied 1)"
    # -> 5-0/5-0-0 and 4-0-1 either way around (live rejections
    # 2026-09-20).
    for match in re.finditer(
        r"home[-_](?:team[-_])?higher[-_ ](?:count[-_ ])?(\d+)"
        r"\W{1,4}away[-_](?:team[-_])?higher[-_ ](?:count[-_ ])?(\d+)"
        r"\W{1,4}tied[-_ ](?:count[-_ ])?(\d+)",
        lowered,
    ):
        home, away, tied = (int(group) for group in match.groups())
        prose_record(home, away, tied)
        prose_record(away, home, tied)
    # Unanimity states the zero implicitly: "all 5 forecasters rank
    # Dallas higher" -> 5-0 and 5-0-0.
    for match in re.finditer(r"all (\d+) forecasters", lowered):
        prose_record(int(match.group(1)), 0, 0)
    # The prior-win comparison: "7 prior-season wins to las vegas's 3"
    # -> 7-3 either way around.
    for match in re.finditer(
        r"(\d+) prior-season wins to [^\",.]{0,40}?(\d+)", lowered
    ):
...
def _check_reason_citations(text: str, reference: str, *, field: str) -> None:
    ...
    word "game", allowing up to two intervening words on the count-first
    side — the request writes "has 0 historical games" and a reason
    compresses it to "0 games" (a live rejection, 2026-09-20).

    A strictly ascending two-part token whose endpoints both appear as
    numbers in the reference reads as a range, not a record — "3-5 game
    samples", "44-46 projections", the "2-3 prior-win-gap bucket" — and
    ranges over request numbers are how a reason compresses cohort sizes
    and projections, so they pass (11 live rejections on 2026-09-13).
    Descending pairs and W-L-T stay verbatim-only: an invented winning
    record or tie tally is still caught.
    """
    claim = _reference_form(text)
    range_endpoints: set[str] | None = None
    for match in _RECORD_PATTERN.finditer(claim):
        if match.group(0) not in reference:
            first, second, third = match.groups()
            if third is None and int(first) < int(second):
                ...
                if (
                    first in range_endpoints
                    and second in range_endpoints
                ):
                    continue
            raise ValueError(
    ...
    for match in _GAME_COUNT_PATTERN.finditer(claim):
        ...
        if (
            re.search(
                rf"game\w*\W{{0,12}}\b{count}\b"
                rf"|\b{count}\b(?:\W+\w+){{0,2}}\W{{0,12}}game",
                reference,
            )
            is None
        ):
