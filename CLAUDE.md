...
**Acknowledge every inbound Telegram message with a reaction before starting work.** The first thing to do on any incoming channel message is call the `react` tool (👀) on that `message_id`. This is a delivery receipt: the user can see the session received the message. Its **absence** is the signal that a message was dropped — which happens because the Bot API has **no history/backfill**, so any message sent during a session restart window (before the new process's poll loop is connected) is silently lost. Note the resume-notify hook's "▶️ Restarted… copy this back to resume" message is posted by the SessionStart hook and does **not** prove the receive loop is ready — the user should wait for a reaction/reply to a fresh message before firing the real task. The reaction is detection, not prevention; a dropped message still needs to be resent.
