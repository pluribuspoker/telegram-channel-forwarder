...
- **Rating voice `rating_elo` (WP7):** `mode: model`, deterministic backend, schema 9, `default_model: deterministic`, `markets: [side]` (its total is the league scoring rate). Prior `moe/priors/nfl_elo_v1.json` from `scripts/fit_nfl_elo.py` (pure stdlib; K 19, hfa 32, regression ⅓; 2025 Brier 0.2224 vs closing ML 0.2116, the within-0.01 target missed by 0.0008, `target_met: false`); `build_rating_input` refuses any season but prior+1 — refit each offseason. Weekly: `python scripts/generate_rating_week.py --season S --week N` → `python scripts/review_moe_opinion.py --expert rating_elo --week N --season S --reviewed-by <you>` (table) → same with `--approve`; nothing approves on its own. **With `rating_elo` enabled the judge runner needs an approved rating row per game** — generate and approve the week before the timer's pass, or it skips every game as "committee incomplete". `data/nfl_lines_history.csv` now spans 1999–2025 (`DEFAULT_SEASONS`). Test: `scripts/test_moe_rating.py`.
- **Test protocol:** `bash scripts/godbuild_test.sh <slug> <worktree-dir> [extra modules]` — fresh scratch clone on the VPS, overlay of everything that differs from origin/main, the ten God Expert modules, clone removed. Never `~/app`, never local (fcntl).
