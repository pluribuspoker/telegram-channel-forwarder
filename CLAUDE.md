...
- **Test protocol:** `bash scripts/godbuild_test.sh <slug> <worktree-dir> [extra modules]` — fresh scratch clone on the VPS, overlay of everything that differs from origin/main, the eleven God Expert modules, clone removed. Never `~/app`, never local (fcntl).
