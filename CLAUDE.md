...
Pinned offline by `scripts/test_total_grade_math.py`, built on the two real box scores. Validated by replaying every cached pick against its stored verdict: **196 agree, 1 disagrees, 26 decline** — and the one disagreement is the run-line PUSH above. The 26 are UFC moneylines and CFL spreads, both correctly refused (CFL scores are scraped, not on the ESPN scoreboard). Re-run that replay after touching this function; agreement should stay at ~100% minus known defects.
