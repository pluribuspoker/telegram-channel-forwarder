...
- **Retryable misses aren't permanent anymore**: `no_game`/`no_*_data` re-fetched by the tracker with free sources only, every ≥30 min until game start (`should_retry_odds`, bounded by `commence_time`/`game_date`) — Bovada lists period markets (MLB F5, CFL quarters) only in a pregame window the first attempt can miss. Priced/live/structural results stay final; retry misses don't re-warn the audit channel.
