...
- Registry `moe/experts.yaml`; every opinion is an append-only row in the `moe_opinions` tab of the NFL Guesser sheet (creds VPS-only), approved hash-bound via `scripts/review_moe_opinion.py`; the dedicated `@nfl_moe_bot` displays them in the private MOE group. Generate with `scripts/generate_moe_opinion.py` — agent path via the `generate-nfl-moe-opinion` skill (subscription billing); `--api` is an explicit fallback.
...
- **Dedicated MOE bot — `moe_bot.py` + `moe_desk.py`:** `@nfl_moe_bot` alone owns the private supergroup topics (📥 Review · 🏈 Picks · 📊 Scores); `@nflguesser_bot` has no MOE view, callback, deep link, or sync loop. Picks and Review keep compact primary cards and post full persisted opinions as same-group replies, split below Telegram's limit and edited in place. ✅/❌ callbacks are checked against the `reviewer` role in `allowed_users`, re-read the sheet, refuse anything not pending, and run the store's hash-checked review signed with the tapper's display name. Loud only for a new bet leg and a near judge lock; `GOD_JUDGE_PENDING_DM=0` silences the runner's pending DM. State is `moe_bot_state.json` (gitignored, pruned 3 days after kickoff). `MOE_BOT_TOKEN`/`MOE_BOT_SESSION` are server-only in `.env.local`; group/topic IDs remain in `.env`. Setup: `scripts/desk_setup.py --create-topics | --grant-reviewer <id> | --check [--post-test]`; service: `moe-bot.service`. Tests: `scripts/test_moe_desk.py`, `scripts/test_moe_bot.py`, `scripts/test_moe_identity.py`.
