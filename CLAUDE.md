...
- **Celebrity Consensus Expert:** Opus-only, `committee_optional` side+total voice over the celebrities who actually picked a game. `celebrity_picks` stores canonical structured fields plus exact raw text, keeps the latest revision per distinct bet, and tracks props/other markets without treating them as direct side/total evidence. A PASS leg is excluded from that God pool.
