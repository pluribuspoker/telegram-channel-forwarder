...
- `god-judge.timer` — God Expert judge runner (`scripts/god_judge_runner.py` via `run_god_judge.sh`), every 30 min at :12/:42, `TimeoutStartSec=3600`. Per upcoming NFL game with a complete committee (an approved row for every enabled non-aggregator expert) and more than two hours to kickoff it builds one aggregator input, persists the rules row, makes ONE headless `claude -p --safe-mode --tools ""` call (Fable 5.1, max effort, the registered prompt as the system prompt, the masked request on stdin, an empty cwd, an environment stripped to PATH/HOME/the OAuth token), persists the judge row (backend `claude_headless`), and DMs the operator the review commands through the watchdog bot. Dedupe by committee key (voice opinion ids + latest lines/prices, never the timestamp): a pending or approved judge row blocks, a rejected one re-runs, two invalid rows stall the key until a voice or price changes; `--max-games 3` per pass; a failed call is logged and DMed, never retried. Logs: `journalctl -u god-judge`, `/tmp/god_judge_last_run.log`, usage ledger `~/app/logs/god_judge_runs.jsonl` (one line per call: duration, cost, turns). Plan without spending: `su - forwarder -c "cd ~/app && ~/venv/bin/python scripts/god_judge_runner.py --dry-run"`. Auth: the unit loads `~/.claude/auth.env` (the same 1-year `CLAUDE_CODE_OAUTH_TOKEN` the channels session uses); when that token dies the runner fails row-less every pass and DMs — fix with `/reauth` → `/authcode`. **`--safe-mode` is load-bearing:** it disables plugins, so this second CLI process never starts the Telegram plugin's bun poller (the 2026-09-07 probe left `claude-channels.service` untouched, unlike the bare CLI probe that the auth watchdog avoids), and `--bare` would be wrong because it never reads OAuth. Healthcheck: `GOD_JUDGE_HEALTHCHECK_URL` in `.env` (synced; `ping_hc` no-ops unset). Never run the judge from an interactive session that has seen unmasked committee rows.
- `moe-grade.timer` — MOE opinion grader (`scripts/moe_grade.py --write --notify` via `run_moe_grade.sh`), daily at 05:23 ET (the VPS clock is America/New_York: after the last Thu/Sun/Mon night final, before the 06:00 auto-reboot and ~06:31 unattended-upgrade windows, off the tracker's :20 slot), `Persistent=true`, `TimeoutStartSec=1200`. Deterministic — no Claude call, no Telethon: per run four Sheets reads (`nfl_game_history`, `moe_opinions`, `nfl_line_snapshots`, the `moe_grades` opinion-id column), two ESPN scoreboard fetches, one append when there is something new. Idempotent (the ledger dedupes on opinion id), so it runs every day rather than only after game days: approvals can lag a game by days, and an idle run is four reads and an exit. DMs the operator the scoreboard through the watchdog bot only when rows were appended (season header, the finals covered, one line per expert); a failed DM after a successful append exits non-zero so the healthcheck alerts — the append never repeats, so the DM is the only signal. The judge runner recomputes the scoreboard live from finals and snapshots, so this ledger feeds humans and the disagreement report, never the Hedge weights. Logs: `journalctl -u moe-grade`, `/tmp/moe_grade_last_run.log`. Manual, read-only: `su - forwarder -c "cd ~/app && ~/venv/bin/python scripts/moe_grade.py"` (`--json` for the full report). Healthcheck: `MOE_GRADE_HEALTHCHECK_URL` in `.env` (synced; `ping_hc` no-ops unset). Off-season: `sudo systemctl disable --now moe-grade.timer`, or leave it running.
