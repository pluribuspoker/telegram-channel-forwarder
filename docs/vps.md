...
- `telegram-intake.service` — `@nflguesser_bot`, prediction and source-input workflows only. It does not display or operate MOE opinions.
- `moe-bot.service` — `@nfl_moe_bot`, the private MOE supergroup writer. Runs desk synchronization, channel-native full opinion replies and reviewer callbacks. `MOE_BOT_TOKEN`/`MOE_BOT_SESSION` are server-only in `.env.local`; state is `moe_bot_state.json`.
