...
### Player props under the free tier

Props are **2% of historical picks** (8 of 381, and half were stats no source ever supported: NRFI, KO/TKO, 1st-inning pitches). They stay on the paid path as last resort (`PROP_STAT_MARKETS` gates which stats are even attempted; a supported prop costs ~2–3 credits — trivial inside 500/mo). **Bovada's coupon does carry player-prop groups on the same endpoint we already fetch** (observed on CFL 2026-08-22: TD Scorer/Passing/Rushing/Receiving groups, player name inside the market description, e.g. "Total Passing Yards - Nathan Rourke (BC)") — that is the future free path if prop volume ever grows, but it needs per-sport stat→market-title maps built from observed payloads, unjustified at today's volume. `free_only` prop attempts short-circuit to `player_prop_unavailable` without spending anything.
