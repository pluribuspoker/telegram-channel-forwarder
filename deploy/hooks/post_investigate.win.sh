...
# invisible to Claude).
#
# Windows / Git Bash port of post_investigate.sh.
# Differences from the VPS (Linux) version:
#   - `python` instead of `python3` (Windows has no bare python3)
#   - /tmp maps to %TEMP% via Git Bash (works transparently)
#   - md5sum is provided by Git Bash (/usr/bin/md5sum)
#
...
COUNT=$(TRANSCRIPT="$TRANSCRIPT" python <<'PYEOF'
import os, json
try:
