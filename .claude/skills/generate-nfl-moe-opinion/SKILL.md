...
Total, AK, and Cee Experts, and the manual fallback for the God Expert judge
...
2. Produce the exact deterministic input without inference:
   ...
   python scripts/generate_moe_opinion.py \
     --event-id <event-id> --expert <schedule|divisional|win_total|ak|cee> \
...
7. Persist through the normal pipeline. Only output-schema-v4 responses use a
   ...
   ```bash
   # Schedule, Win Total, AK, or Cee Expert
   python scripts/generate_moe_opinion.py \
     --event-id <event-id> --expert <schedule|win_total|ak|cee> \
