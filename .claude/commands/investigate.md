...
7. **"Didn't run" usually means "ran and crashed."** For a scheduled job (cron/timer) that "sometimes doesn't fire," read its output log (`/tmp/*_cron.log`, `journalctl -u <unit>`) for a traceback BEFORE suspecting the scheduler. The job often ran fine and died late in the pipeline (e.g. Playwright screenshot stall under VPS memory pressure — ~1GB RAM, no swap). The fix is resilience (retry + explicit timeouts), not the schedule.
8. **"Parsed as UNKNOWN" is usually a coverage gap, not a parse failure.** When a pick parses correctly (right sport/teams/bet_type) but grades UNKNOWN, suspect the score fetcher's competition list, not the parser. UNKNOWN comes from `build_context` returning `CONTEXT_SKIP`, which for Soccer means `fetch_soccer_context` found no game — often because the league isn't in `SOCCER_LEAGUES` (it only queried the top-5 + UEFA + World Cup; Allsvenskan/Eliteserien/etc. were missing). Verify by hitting the ESPN scoreboard for the right league code (`soccer/swe.1`, `nor.1`, …) directly — if ESPN has the game, add the league code to the fetcher's list.
...
10. **A "sent twice" duplicate is often a source-side delete-and-repost, not a listener race.** The listener's dedup keys on `(channel, dest, msg_id)` (`_in_flight` + `listener_forwarded` table). When a capper deletes a pick and re-posts it (or double-taps send), the repost gets a **new msg_id**, so both copies pass the id-based guards and forward. Ground truth is the `listener_forwarded` table (`sqlite3 picks.db`) — query by dest to see every source msg_id forwarded; a duplicate shows up as **two adjacent source ids** (e.g. 430800 + 430801) with the same content and near-identical `ts`. Confirm by fetching each source id directly (`get_messages(src, ids=<id>)`, NOT topic-filtered — a deleted or reposted id may be invisible to a topic-scoped fetch): the earlier one usually returns `<none>` (deleted at source). Fix is content-dedup, not id-dedup.
...
13. **A hard parse failure (`_failed: true`, "parse failed" UNKNOWN) is often valid JSON the extractor couldn't lift out of prose — NOT the model failing to understand the pick.** Before blaming the prompt or the model's comprehension, reproduce `claude_parse` and print the **raw** `resp.content[0].text`. Sonnet wraps its JSON in a reasoning preamble when the message is ambiguous (e.g. "OVER 3.5 IN THE FIESTA BOWL" — a soccer total nicknamed after a football bowl); `json.loads` then chokes on the leading prose and `_salvage_truncated` (built for *truncated* output) can't recover it → `None` → `_failed`. The model usually got the pick right; the JSON was there all along. `claude_parse` now runs `_extract_first_json_object` (balanced-brace scan) before salvage, so this specific shape is handled — but the debugging move (capture raw output to separate an *extraction* failure from a *comprehension* failure) generalizes to any LLM→JSON pipeline. Corollary: when a bet slip photo is attached and the **text** parse is ambiguous/incomplete (`sport=Other` or empty `teams`), the slip is ground truth — the tracker's image-consultation trigger (`_is_text_thin` / `_parlay_structure_uncertain` / `_parse_incomplete`) re-parses from the image.
